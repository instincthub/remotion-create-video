...
import { cyanShadow } from "./theme";
import type { Section, SectionState } from "./types";

/**
 * Persistent right-rail progress tracker — the spine of the lesson.
 *
 * A CLEAN roadmap (no blur): every section is always readable.
 *   • active   → gunmetal fill, white label, DARKCYAN number badge (white text —
 *                the brand's primary-button pattern)
 *   • done     → dimmed with a Caribbean-green checkmark (the success token)
...
const TrackerCard: React.FC<{
  ...
  return (
    ...
    >
      {/* Number / check badge — DarkCyan + white text when active */}
      <div
        style={{
          flexShrink: 0,
          width: 46,
      ...
      >
        {isDone ? <Check color={colors.green} /> : String(section.n).padStart(2, "0")}
      </div>
