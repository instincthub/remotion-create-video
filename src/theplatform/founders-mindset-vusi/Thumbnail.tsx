...
/**
 * Course thumbnail (16:9) for "The Founder's Mindset: Making Ideas Happen".
 * The Platform identity — yellow canvas, ink bars, the lightbulb motif — with
 * Vusi's portrait (a frame lifted from the keynote footage). Rendered locally
 * via `npx remotion still fm-thumbnail`; nothing leaves the machine.
 *
 * portraitSrc defaults to the public/ frame; pass another to reuse the layout.
 */
export const Thumbnail: React.FC<{ portraitSrc?: string }> = ({
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: colors.yellow, fontFamily: overpass }}>
      ...
      <div style={{ position: "absolute", left: 90, top: 196, width: 1040 }}>
        <span
          style={{
      ...
      {/* speaker credit */}
      <div style={{ position: "absolute", left: 96, bottom: 86, display: "flex", alignItems: "center", gap: 22 }}>
        <Bulb size={64} stroke={colors.ink} spark={colors.ink} lit={0} strokeWidth={5} />
        <div>
          <div style={{ fontWeight: 900, fontSize: 54, letterSpacing: -1.5, color: colors.ink, lineHeight: 1 }}>
            Vusi Thembekwayo
          </div>
          <div style={{ fontWeight: 600, fontSize: 32, color: colors.ink, opacity: 0.78, marginTop: 6 }}>
            Founder &amp; CEO, My Growth Fund Venture Partners
          </div>
        </div>
      </div>
